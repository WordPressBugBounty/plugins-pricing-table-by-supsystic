...
class Twig_Environment
{
  ...
  public function createTemplate($template)
  {
    ...
    $loader = new Twig_Loader_Chain([new Twig_Loader_Array([$name => $template]), ($current = $this->getLoader())]);
    ...
    foreach ($this->getExtensions() as $extension) {
      if (!$extension instanceof Twig_Extension_InitRuntimeInterface) {
        ...
        if ('Twig_Extension' !== $m->getDeclaringClass()->getName()) {
          @trigger_error(
            sprintf('Defining the initRuntime() method in an extension is deprecated. Use the `needs_environment` option to get the Twig_Environment instance in filters, functions, or tests; or explicitly implement Twig_Extension_InitRuntimeInterface if needed (not recommended).', $name),
            E_USER_DEPRECATED,
          );
        }
      }
