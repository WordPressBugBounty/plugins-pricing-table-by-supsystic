...
class framePts
{
  ...
  {
    $this->_res = toeCreateObjPts('response', []);
  }
  ...
  public function _delayedInit()
  {
    ...
    register_activation_hook(PTS_DIR . DS . PTS_MAIN_FILE, ['utilsPts', 'activatePlugin']);
    register_uninstall_hook(PTS_DIR . DS . PTS_MAIN_FILE, ['utilsPts', 'deletePlugin']);
    register_deactivation_hook(PTS_DIR . DS . PTS_MAIN_FILE, ['utilsPts', 'deactivatePlugin']);
  ...
  public function isAdminPlugOptsPage()
  {
    ...
    if (is_admin() && strpos((string) $page, framePts::_()->getModule('adminmenu')->getMainSlug()) !== false) {
      return true;
    }
    return false;
  }
