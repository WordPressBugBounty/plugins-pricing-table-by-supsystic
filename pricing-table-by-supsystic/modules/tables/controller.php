...
class tablesControllerPts extends controllerPts
{
    ...
    if (!empty($data)) {
      foreach ($data as $i => $v) {
        ...
        if (!empty($data[$i]['label']) && is_string($data[$i]['label'])) {
          if (
            (strpos($data[$i]['label'], 'script') !== false || strpos($data[$i]['label'], 'getscript') !== false || strpos($data[$i]['label'], '$') !== false || strpos($data[$i]['label'], 'jquery') !== false) &&
  ...
  public function importJSONTable()
  {
    ...
    $tables = [];
    $requiredFields = ['css', 'html', 'img', 'img_url', 'is_base', 'is_pro', 'original_id', 'params', 'label'];
    if (!count($data)) {
  ...
  public function getPermissions()
  {
    return [
      PTS_USERLEVELS => [
        PTS_ADMIN => ['getListForTbl', 'remove', 'removeGroup', 'clear', 'save', 'exportForDb', 'updateLabel', 'changeTpl', 'saveAsCopy', 'getJSONExportTable', 'importJSONTable', 'createFromTpl'],
      ],
    ];
  }
}
