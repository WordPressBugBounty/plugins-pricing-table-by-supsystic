...
class tablesModelPts extends modelPts
{
  ...
  protected function _afterGetFromTbl($row)
  {
    ...
    $row['params'] = isset($row['params']) && !empty($row['params']) ? utilsPts::unserialize(base64_decode($row['params']), true) : [];
    $row['params'] = $this->_afterDbReplace($this->_afterDbParams($row['params']));
    $row = $this->_afterDbReplace($row);
    $row['img_url'] = isset($row['img']) && !empty($row['img']) ? $imgsPath . $row['img'] : $imgsPath . strtolower(str_replace([' ', '.'], '-', $row['label'])) . '.jpg';
    $row['id'] = (int) $row['id'];
