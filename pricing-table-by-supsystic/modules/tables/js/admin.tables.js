jQuery(document).ready(function () {
  ...
  if (typeof ptsOriginalTable !== 'undefined') {
    // Just changing template - for existing table
    ptsInitChangeTableDialog();
  } else {
    // Creating new table
    ptsInitCreateTableDialog();
  }
  if (jQuery('.ptsTplPrevImg').length) {
    // If on creation page
  ...
  jQuery('.table-list-item:not(.sup-promo)').click(function () {
    ...
    if (ptsOriginalTable.original_id == id) {
      var dialog = jQuery('<div />')
        .html(toeLangPts('This is same template that was used for Table before'))
        .dialog({
          modal: true,
          width: 480,
          height: 180,
