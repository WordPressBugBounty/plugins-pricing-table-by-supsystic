!(function ($) {
  $(function () {
    ...
    if ($startPage.length) $startPage.show();
    ...
    $root.find('#imex-export').click(function () {
      ...
      $exportTable.find('.jqgrow[aria-selected="true"]').each(function () {
        ...
        if (tableID) selectedExportTable.push(tableID);
      });
      ...
      if (selectedExportTable.length) {
        jQuery.sendFormPts({
          ...
          onSuccess: function (res) {
            ...
            if (data) {
              $page.find('textarea').val(JSON.stringify(data));
    ...
    function navigateTo(element) {
      ...
      if (element instanceof jQuery) $openPage = element;
      else $openPage = jQuery(element);
