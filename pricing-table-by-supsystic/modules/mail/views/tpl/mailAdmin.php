<form id="ptsMailTestForm">
	<label>
		...
		<?php echo htmlPts::text('test_email', ['value' => $this->testEmail]); ?>
	</label>
	<?php echo htmlPts::hidden('mod', ['value' => 'mail']); ?>
	<?php echo htmlPts::hidden('action', ['value' => 'testEmail']); ?>
   ...
   ?>
			<tr>
				<th scope="row" class="col-w-30perc">
					...
					<?php if (!empty($opt['changed_on'])) { ?>
						...
						<span class="description">
							<?php $opt['value'] ? printf(__('Turned On %s', PTS_LANG_CODE), datePts::_($opt['changed_on'])) : printf(__('Turned Off %s', PTS_LANG_CODE), datePts::_($opt['changed_on'])); ?>
						</span>
					<?php } ?>
				</th>
				...
				<td class="col-w-1perc">
					<?php echo htmlPts::$htmlType('opt_values[' . $optKey . ']', ['value' => $opt['value'], 'attrs' => 'data-optkey="' . $optKey . '"']); ?>
				</td>
	...
	<?php echo htmlPts::hidden('mod', ['value' => 'mail']); ?>
	<?php echo htmlPts::hidden('action', ['value' => 'saveOptions']); ?>
	<button class="button button-primary">
