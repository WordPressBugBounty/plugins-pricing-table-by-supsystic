...
function ptsCallWpMedia(params) {
  params = params || {};
  if (typeof wp !== 'undefined' && wp.media && wp.media.editor) wp.media.editor.open(params.id);
  ...
  wp.media.editor.send.attachment = function (opts, attach) {
    ...
    var imgUrl = opts.size && attach.sizes[opts.size] && attach.sizes[opts.size].url ? attach.sizes[opts.size].url : attach.url;
...
function ptsMceMoveToolbar(editor, clientX) {
  ...
  if (newY <= 150) {
    // Panel at the bottom of the element
...
ptsChangeElAttrs.prototype._initMenuItem = function () {
  ...
  $newEl = $newEl.clone().removeAttr('id').addClass('ptsElMenuBtn');
  $parent.prepend('');
...
ptsChangeElAttrs.prototype._initSubMenuItem = function () {
  ...
  $newEl = $newEl.clone().removeAttr('id');
  $parent.append($newEl);
...
ptsChangeElAttrs.prototype._getAttrToSubMenu = function () {
  var me = this,
    self = me._scope,
    $el = self._element._$.hasClass('ptsEl') ? self._element._$ : self._element._$.find('.ptsEl').eq(0),
  ...
  $el.each(function () {
    ...
    for (var key in allowedAttr) {
      ...
      if (typeof allowedAttr[key] == 'string') {
        ...
        $newEl = jQuery('#ptsRowAddHtmlAttr').clone().removeAttr('id');
