...
window.onbeforeunload = function () {
  ...
  if (ptsAdminFormChanged.length) return 'Some changes were not-saved. Are you sure you want to leave?';
};
...
function changeAdminFormPts(formId) {
  if (jQuery.inArray(formId, ptsAdminFormChanged) == -1) ptsAdminFormChanged.push(formId);
}
...
function ptsInitStickyItem() {
  jQuery(window).scroll(function () {
    ...
    if (wasSticking) {
      if (jQuery('#ptsPopupGoToTop').length) jQuery('#ptsPopupGoToTop').show();
    } else if (wasUnSticking) {
      if (jQuery('#ptsPopupGoToTop').length) jQuery('#ptsPopupGoToTop').hide();
    }
  });
}
function ptsGetTxtEditorVal(id) {
  if (typeof tinyMCE !== 'undefined' && tinyMCE.get(id) && !jQuery('#' + id).is(':visible')) return tinyMCE.get(id).getContent();
  else return jQuery('#' + id).val();
}
function ptsSetTxtEditorVal(id, content) {
  if (typeof tinyMCE !== 'undefined' && tinyMCE && tinyMCE.get(id) && !jQuery('#' + id).is(':visible')) tinyMCE.get(id).setContent(content);
