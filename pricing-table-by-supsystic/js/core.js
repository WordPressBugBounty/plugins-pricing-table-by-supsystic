if (typeof PTS_DATA == 'undefined') var PTS_DATA = {};
if (isFinite(PTS_DATA.animationSpeed)) PTS_DATA.animationSpeed = parseInt(PTS_DATA.animationSpeed);
else if (jQuery.inArray(PTS_DATA.animationSpeed, ['fast', 'slow']) == -1) PTS_DATA.animationSpeed = 'fast';
...
jQuery.fn.sendFormPts = function (params) {
  ...
  if (msgEl && !params.btn) {
    jQuery(msgEl).removeClass(params.errorClass).removeClass(params.successClass);
  ...
  if (params.btn) {
    ...
    if (jQuery(params.btn).find('.fa').length) {
      if (!btnWasDisabled) {
        params.btnIconElement.data('prev-class', params.btnIconElement.attr('class'));
      }
  ...
  else if (typeof ajaxurl == 'undefined') url = PTS_DATA.ajaxurl;
  else url = ajaxurl;
...
function toeProcessAjaxResponsePts(res, msgEl, form, sentFromForm, params) {
  ...
  if (typeof res == 'object') {
    ...
    if (res.error) {
      if (msgEl) {
        jQuery(msgEl).removeClass(params.successClass).addClass(params.errorClass);
      }
      var errorsArr = [];
      for (var name in res.errors) {
        ...
        if (jQuery('.ptsErrorForField.toe_' + nameToClassId(name) + '').exists())
          jQuery('.ptsErrorForField.toe_' + nameToClassId(name) + '')
            .show()
      ...
      if (errorsArr.length && params.btn && jQuery.fn.dialog) {
        jQuery('<div title="' + toeLangPts('Really small warning :)') + '" />')
      ...
      if (msgEl) {
        jQuery(msgEl).removeClass(params.errorClass).addClass(params.successClass);
...
function toeOptionPts(key) {
  if (PTS_DATA.options && PTS_DATA.options[key]) return PTS_DATA.options[key];
  return false;
}
function toeLangPts(key) {
  if (PTS_DATA.siteLang && PTS_DATA.siteLang[key]) return PTS_DATA.siteLang[key];
  return key;
}
function toePagesPts(key) {
  if (typeof PTS_DATA != 'undefined' && PTS_DATA[key]) return PTS_DATA[key];
  return false;
}
/**
...
function toeOptTimeoutHideDescriptionPts() {
  ...
  setTimeout(function () {
    if (!jQuery('#ptsOptDescription').attr('toeFixTip')) toeOptHideDescriptionPts();
  }, 500);
}
/**
...
function toeOptShowDescriptionPts(description, x, y, moveToLeft) {
  if (typeof description != 'undefined' && description != '') {
    ...
    else jQuery('#ptsOptDescription').css('left', x + 10);
